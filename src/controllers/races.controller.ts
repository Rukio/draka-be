...
const getInactiveByUuid = async (req, res, next) => {
	try {
		const raceData = await service.getManyWithParagraphs(
		...
		if (!race) {
			return next(error.notFound("Error while getting a race"));
		}
		res.json(race);
	} catch (err) {
		console.log("Error getting a race", err);
		next(err);
	}
};
